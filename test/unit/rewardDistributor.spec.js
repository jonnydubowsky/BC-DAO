...
describe("RewardsDistributor", async () => {
  ...
  it("does no distribution if stake becomes ineligible after withdrawl", async function() {
    ...
    await rd.methods
      .deposit(acct_a, amountDeposit.toString())
      .send({ from: creator });

    tx = await rd.methods
      .withdrawStake(acct_a, amountWithdraw.toString())
      .send({ from: creator });
  ...
  it("allocates all reward to a single staker and allow its withdrawl", async function() {
    ...
    tx = await rd.methods
      .distribute(creator, amountDistribute.toString())
      .send({ from: creator });
  ...
  it("allocates no reward to stake <= ELIGIBLE_UNIT", async function() {
    ...
    await rd.methods
      .distribute(creator, distribute1.toString())
      .send({ from: creator });
  ...
  it("allocates 1st reward proportionally to 2 stakers and 2nd reward to remaining staker after the other withdrew", async function() {
    ...
    await rd.methods
      .deposit(acct_a, depositA.toString())
      .send({ from: creator });
    await rd.methods
      .deposit(acct_b, depositB.toString())
      .send({ from: creator });
    await rd.methods
      .distribute(creator, distribute1.toString())
      .send({ from: creator });
    ...
    await rd.methods.withdrawAllStake(acct_a).send({ from: creator });
    await rd.methods
      .distribute(creator, distribute2.toString())
      .send({ from: creator });
  ...
  it("distributes after partial stake withdrawal and reads reward", async function() {
    ...
    tx = await rd.methods
      .withdrawStake(acct_b, withdrawB.toString())
      .send({ from: creator });
    expectEvent.inLogs(tx.events, "StakeWithdrawalMade", {
      to: acct_b,
      value: withdrawB
  ...
  it("withdraws reward after stake has been withdrawn", async function() {
    ...
    tx = await rd.methods
      .withdrawStake(acct_a, depositA.toString())
      .send({ from: creator });
    expectEvent.inLogs(tx.events, "StakeWithdrawalMade", {
      to: acct_b,
      value: depositA
  ...
  it("handles magnitude: deposit 10**6 10**9 10**12 10**15, distribute, withdraw reward", async function() {
    ...
    await rd.methods
      .deposit(acct_d, String(10 ** 15 - 10 ** 12 - 10 ** 9))
      .send({ from: creator });

    await rd.methods
      .distribute(creator, String(10 ** 9))
      .send({ from: creator });
