...
describe("BondingCurveFactory", async () => {
  let tx;
  let result;
  ...
  beforeEach(async function() {
    ...
    bondedTokenImpl = await deploy.getImplementation(
      project,
  ...
  it("should have parameters initialized correctly", async function() {
    ...
    expect(result.bancorCurveServiceImpl).to.be.equal(
      bancorCurveService.address
    );
  });
  ...
  describe("Deploy StaticCurveLogic version", async () => {
    ...
    describe("Deploy", () => {
      ...
      beforeEach(async function() {
        const createdEvent = expectEvent.inLogs(
          deployTx.events,
          "BondingCurveDeployed"
        );

        const deployedContracts = await getContractsFromDeployedEvent(
          createdEvent
        );
      ...
      it("should deploy contracts on deploy", async function() {
        ...
        expect(
          await web3.eth.getCode(bondingCurve.options.address)
        ).to.not.be.equal(nonContractCode);
        expect(
          await web3.eth.getCode(bondedToken.options.address)
        ).to.not.be.equal(nonContractCode);
        expect(
          await web3.eth.getCode(buyCurve.options.address)
        ).to.not.be.equal(nonContractCode);
        expect(
          await web3.eth.getCode(sellCurve.options.address)
        ).to.not.be.equal(nonContractCode);
        expect(
          await web3.eth.getCode(rewardsDistributor.options.address)
        ).to.not.be.equal(nonContractCode);
      });
      ...
      it("should correctly initialize bonded token parameters", async function() {
        expect(
          await bondedToken.methods.name().call({ from: miscUser })
        ).to.be.equal(deployParams.bondedTokenName);
        expect(
          await bondedToken.methods.symbol().call({ from: miscUser })
        ).to.be.equal(deployParams.bondedTokenSymbol);
        expect(
          new BN(await bondedToken.methods.decimals().call({ from: miscUser }))
        ).to.be.bignumber.equal(new BN(18));
        expect(
          new BN(
            await bondedToken.methods.totalSupply().call({ from: miscUser })
          )
      ...
      it("should correctly initialize reward distributor parameters", async function() {
        expect(
          await rewardsDistributor.methods.owner().call({ from: miscUser })
        ).to.be.equal(bondedToken.options.address);
      });
