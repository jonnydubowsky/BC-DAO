...
async function shouldBehaveLikeBondingCurve(context, parameters) {
  ...
  beforeEach(async function() {
    ...
    const paymentTokenInitialBalance = new BN(
    ...
    rewardsDistributor = await deploy.deployRewardsDistributor(project, [
      curveOwner
    ]);
  ...
  describe("Initialization", async () => {
    it("should have properly initialized parameters", async function() {
      expect(
        await bondingCurve.methods.owner().call({ from: miscUser })
      ).to.be.equal(curveOwner);
      expect(
        await bondingCurve.methods.beneficiary().call({ from: miscUser })
      ).to.be.equal(curveOwner);
      expect(
        await bondingCurve.methods.collateralToken().call({ from: miscUser })
      ).to.be.equal(paymentToken.address);
      expect(
        await bondingCurve.methods.bondedToken().call({ from: miscUser })
      ).to.be.equal(bondedToken.address);
      expect(
        await bondingCurve.methods.buyCurve().call({ from: miscUser })
      ).to.be.equal(buyCurve.address);
      expect(
        await bondingCurve.methods.sellCurve().call({ from: miscUser })
      ).to.be.equal(sellCurve.address);
      expect(
  ...
  describe("Curve Admin", async () => {
    ...
    it("should allow owner to set new owner", async function() {
      ...
      expect(
        await bondingCurve.methods.owner().call({ from: newOwner })
    ...
    it("should allow owner to set new sell curve", async function() {
      tx = await bondingCurve.methods
        .setSellCurve(constants.ZERO_ADDRESS)
        .send({
          from: curveOwner
        });
      expect(
    ...
    it("should allow owner to set new split on pay", async function() {
      ...
      expect(
        await bondingCurve.methods.splitOnPay().call({ from: miscUser })
      ).to.be.equal(newSplitOnPay);
    });
  ...
  describe("Buy / Sell", async () => {
    ...
    it("should not allow other addresses to mint bondedTokens", async function() {
      await expectRevert.unspecified(
        bondedToken.methods
          .mint(userAccounts[3], 100)
          .send({ from: userAccounts[3] })
      );
    });
    ...
    describe("Buy Failure Cases", async () => {
      it("should not allow to buy with 0 tokens specified", async function() {
        await expectRevert.unspecified(
          bondingCurve.methods
            .buy(0, maxBuyPrice.toString(), buyer)
            .send({ from: buyer })
        );
      });
      ...
      it("should not allow to buy if sell curve value is higher than buy curve value", async function() {
        ...
        await bondingCurve.methods
          .setSellCurve(buyCurve.address)
          .send({ from: curveOwner });
        await bondingCurve.methods
          .setBuyCurve(sellCurve.address)
          .send({ from: curveOwner });
    ...
    describe("Buy", async () => {
      beforeEach(async () => {
        await paymentToken.methods
          .mint(curveOwner, userBalances.toString())
          .send({
            from: tokenMinter
          });
        await paymentToken.methods
          .mint(buyer, userBalances.toString())
          .send({ from: tokenMinter });
        await paymentToken.methods
          .approve(bondingCurve.address, approvalAmount.toString())
          .send({
            from: curveOwner
          });
        await paymentToken.methods
      ...
      it("should transfer collateral tokens from buyer correctly on buy", async function() {
        ...
        expect(afterBalance).to.be.bignumber.equal(
          beforeBalance.sub(expectedBuyPrice)
        );
      });
      ...
      it("should transfer collateral tokens to reserve correctly on buy", async function() {
        const beforeBalance = new BN(
          await paymentToken.methods
            .balanceOf(bondingCurve.address)
            .call({ from: miscUser })
        );
        ...
        const reserveAmount = new BN(
          expectEvent.getParameter(event, "reserveAmount")
        );

        expect(afterBalance).to.be.bignumber.equal(
          beforeBalance.add(reserveAmount)
        );
      });
      ...
      it("should record reserve balance correctly on buy", async function() {
        ...
        expect(reserveBalance).to.be.bignumber.equal(
          beforeBalance.add(reserveAmount)
        );
      });
      ...
      it("should transfer collateral tokens to beneficiary correctly on buy", async function() {
        const beforeBalance = new BN(
          await paymentToken.methods
            .balanceOf(deployParams.beneficiary)
            .call({ from: miscUser })
        );
        ...
        const beneficiaryAmount = new BN(
          expectEvent.getParameter(event, "beneficiaryAmount")
        );

        expect(afterBalance).to.be.bignumber.equal(
          beforeBalance.add(beneficiaryAmount)
        );
      });
    ...
    describe("Sell Failure Cases", async () => {
      it("should not allow to sell with 0 tokens specified", async function() {
        await expectRevert.unspecified(
          bondingCurve.methods
            .sell(0, maxBuyPrice.toString(), buyer)
            .send({ from: buyer })
        );
      });
    ...
    describe("Sell", async () => {
      ...
      it("should transfer collateral tokens from reserve on sell", async function() {
        ...
        expect(afterBalance).to.be.bignumber.equal(
          beforeBalance.sub(expectedSellReward)
        );
      });
      ...
      it("should transfer collateral tokens to seller on sell", async function() {
        ...
        expect(afterBalance).to.be.bignumber.equal(
          beforeBalance.add(expectedSellReward)
        );
      });
      ...
      it("should allow user to sell and send reward to different recipient", async function() {
        ...
        expect(recipientAfterBalance).to.be.bignumber.above(
    ...
    describe("Payments", async () => {
      ...
      beforeEach(async () => {
        ...
        await paymentToken.methods
          .mint(nonOwner, userBalances.toString())
          .send({
            from: tokenMinter
          });
        await paymentToken.methods
          .approve(bondingCurve.address, paymentAmount.toString())
          .send({
            from: curveOwner
          });
        await paymentToken.methods
      ...
      it("should not allow payments of amount 0", async function() {
        await expectRevert.unspecified(
          bondingCurve.methods.pay(0).send({ from: curveOwner })
        );
      });

      it("should register payments", async function() {
        tx = await bondingCurve.methods
          .pay(paymentAmount.toString())
          .send({ from: nonOwner });
      ...
      describe("Beneficiary / Dividend Split", async () => {
        ...
        const expectedBeneficiaryAmount = paymentAmount
          ...
          .div(maxPercentage);
        const expectedDividendAmount = paymentAmount
          .mul(dividendSplit)
          .div(maxPercentage);
